...
public class SeleniumTests {
	...
	void ieConfig() {
		// chromeDriver
		Path path = Paths.get(System.getProperty("user.dir"), "src/main/resources/chromeDriver/iechromeDriver.exe");
	...
	void runKdErpDownload() {
		...
		// 팝업창 제거
		chromeDriver.executeScript("return document.querySelectorAll('.popupLoginPage').forEach(el => el.remove());");

		// 로그인
		chromeDriverWait.until(ExpectedConditions.presenceOfElementLocated(By.id("txtLoginId"))).sendKeys("m_itsecurity@inveniacorp.com");
		...
		chromeDriver.executeScript("return document.querySelectorAll('.devLoadingArea').forEach(el => el.style.zIndex = '-100');");

		// 구매 메뉴 선택
		chromeDriverWait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("li[moduleseq='7100']"))).click();
		...
		// 구매발주품목 조회
		chromeDriverWait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(By.id("501579_iframe")));
		chromeDriverWait.until(ExpectedConditions.presenceOfElementLocated(By.id("txtCustName_txt"))).sendKeys("디디고");
		//chromeDriverWait.until(ExpectedConditions.presenceOfElementLocated(By.id("datPODateFr_dat"))).sendKeys("2021-02-01");

		chromeDriver.executeScript("return document.getElementById('datPODateFr_dat').value = '2021-02-01';");

		chromeDriverWait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("li[colindex='0']"))).click();
	...
	void runSystemEverDownload(LocalDate fromDate, LocalDate toDate) {
		...
		// 품목 메뉴 선택
		chromeDriverWait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("li[moduleseq='7010']"))).click();
		...
		chromeDriverWait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(By.id("500260_iframe")));
		...
		chromeDriverWait.until(ExpectedConditions.invisibilityOfElementLocated(By.id("divOpenPageLoading")));
		chromeDriverWait.until(ExpectedConditions.invisibilityOfElementLocated(By.className("devLoadingArea")));
		...
		// 거래처 조회
		chromeDriverWait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(By.id("500373_iframe")));
		chromeDriverWait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("li[colindex='1']"))).click();
	...
	String runCozyDownload() throws InterruptedException {
		...
		// 로그인
		chromeDriverWait.until(ExpectedConditions.presenceOfElementLocated(By.id("iEMP_NO"))).sendKeys("damu");
	...
	void runMallDownload() throws InterruptedException, AWTException {
		...
		// 로그인
		ieDriverWait.until(ExpectedConditions.presenceOfElementLocated(By.name("MBR_ID"))).sendKeys("it01");
	...
	private String waitUntilDonwloadCompleted(String windowHanle) throws InterruptedException {
		...
		while (percentageProgress != 100) {
			percentageProgress = (Long) chromeDriver.executeScript("return document.querySelector('downloads-manager').shadowRoot.querySelector('#downloadsList downloads-item').shadowRoot.querySelector('#progress').value");
			System.out.println("Completed Percentage" + percentageProgress);
			Thread.sleep(100);
		}

		String fileName = (String) chromeDriver.executeScript("return document.querySelector('downloads-manager')" +
				".shadowRoot.querySelector('#downloadsList downloads-item')" +
				".shadowRoot.querySelector('div#content #file-link').text");
		String sourceURL = (String) chromeDriver.executeScript("return document.querySelector('downloads-manager')" +
				".shadowRoot.querySelector('#downloadsList downloads-item')" +
				".shadowRoot.querySelector('div#content #file-link').href");
		String donwloadedAt = (String) chromeDriver.executeScript("return document.querySelector('downloads-manager')" +
				".shadowRoot.querySelector('#downloadsList downloads-item')" +
